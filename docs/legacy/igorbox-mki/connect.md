...
:::note
The setup-mode WiFi network's password is printed on the same sticker as the serial number on the bottom of the device.
:::

:::tip
This setup-mode WiFi network only exists until you've finished joining the IgorBox to your real WiFi — once that's done, the IgorBox stops broadcasting it. Complete setup somewhere private (not in a public space), and don't leave a freshly factory-reset IgorBox unattended.
:::
...
If you need to change your wifi credentials, you can use the [Local Management Interface](/docs/legacy/igorbox-mki/local_management) to update the settings.
